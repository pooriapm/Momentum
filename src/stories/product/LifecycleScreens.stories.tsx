...
function lifecycleCopy(locale: SpecLocale, state: LifecycleState): LifecycleCopy {
  const values: Record<LifecycleState, LifecycleCopy> = {
    eligibility: { eyebrow: tx(locale, 'بررسی دسترسی', 'Access check'), title: tx(locale, 'در حال بررسی عضویت و امکان برنامه', 'Checking membership and plan access'), body: tx(locale, 'وضعیت عضویت، بودجه هدیه و مرز ایمنی از منبع معتبر بررسی می‌شوند. نسخه حساب از IP عوض نمی‌شود.', 'Membership, gift budget, and safety boundary are checked from authoritative sources. Account version does not change with IP.'), tone: 'info', icon: <LoaderCircle className="orbit-spin" /> },
    'gift-available': { eyebrow: tx(locale, 'هدیه برنامه اول', 'First-plan gift'), title: tx(locale, 'برنامه اول مهمان Momentum', 'Your first plan is on Momentum'), body: tx(locale, 'بودجه هدیه فعال است. پیش از ساخت برنامه باید روش پرداخت ثبت شود؛ کارت تا چرخه دوم شارژ نمی‌شود.', 'The gift budget is available. A payment method must be on file before generation; the card is not charged until cycle 2.'), tone: 'energy', icon: <Gift /> },
    ...
    checkout: { eyebrow: tx(locale, 'عضویت Momentum', 'Momentum membership'), title: tx(locale, 'روش پرداخت را اضافه کن یا عضو شو', 'Add a payment method, or subscribe'), body: tx(locale, 'برای دریافت هدیه برنامه اول روش پرداخت ثبت کن، یا اشتراک واحد Momentum را شروع کن. کارت تا چرخه دوم شارژ نمی‌شود.', 'Add a card to receive the first-plan gift, or start the one Momentum membership. The card is not charged until cycle 2.'), tone: 'brand', icon: <WalletCards /> },
    ...
    'needs-input': { eyebrow: tx(locale, 'اطلاعات تکمیلی', 'More information needed'), title: tx(locale, 'پیش از شروع یک مورد را اصلاح کن', 'Fix one item before generation'), body: tx(locale, 'روش پرداخت ثبت نشده است. ایمیل، روش پرداخت و کشور صورتحساب فیلدهای جدا هستند. هنوز هیچ درخواست ماهانه‌ای مصرف نشده است.', 'A payment method is missing. Email, payment method, and billing country are distinct named fields. No monthly request has been consumed.'), tone: 'warning', icon: <AlertTriangle /> },
    ...
    imported: { eyebrow: tx(locale, 'ورود موفق', 'Import complete'), title: tx(locale, 'برنامه یک‌ماهه آماده است', 'Your one-month plan is ready'), body: tx(locale, 'دوره از زمان آماده‌شدن برنامه آغاز شد. تاریخ پایان و وضعیت درخواست همیشه در حساب قابل مشاهده‌اند.', 'The period starts when the plan is ready. Its end date and request status stay visible in your account.'), tone: 'success', icon: <BadgeCheck /> },
    ...
    region: { eyebrow: tx(locale, 'نسخه حساب', 'Account version'), title: tx(locale, 'حساب ایرانی قفل شده است', 'This account is the Iranian version'), body: tx(locale, 'رابط فارسی و قیمت ریالی است. ساخت برنامه و هدیه در دسترس‌اند. تغییر IP نسخه را عوض نمی‌کند.', 'The interface is Persian and prices are in rial. Generation and the gift remain available. Changing IP does not switch this version.'), tone: 'brand', icon: <Globe2 /> },
  ...
  return (
    <ProductFrame locale={locale} nav={false} title={tx(locale, 'ساخت برنامه', 'Creating your plan')}>
      <div className="mo-spec__generate-wait">
        ...
        <SpecProgress label={tx(locale, 'آماده‌سازی برنامه یک‌ماهه', 'Preparing the one-month plan')} value={waitProgress[state]} />
    ...
    <ProductFrame locale={locale} nav={false} title={tx(locale, 'چرخه برنامه', 'Plan lifecycle')}>
      ...
      <div className="mo-spec__grid">
        ...
        <SpecCard>
          ...
          <p>{tx(locale, 'یک ماه تقویمی از زمان آماده‌شدن برنامه محاسبه می‌شود.', 'One calendar month is counted from when the plan is ready.')}</p>
...
function LifecycleBody({ locale, state }: { locale: SpecLocale; state: LifecycleState }) {
  ...
  if (state === 'checkout') return <><SpecCallout icon={<WalletCards />} title={tx(locale, 'هدیه یا اشتراک · یک مسیر', 'Gift or membership · one path')} tone="brand">{tx(locale, 'روش پرداخت را برای هدیه برنامه اول ثبت کن، یا اشتراک واحد را شروع کن. قیمت نهایی پیش از هر شارژ و بر اساس نسخه حساب (ریال یا دلار) نمایش داده می‌شود.', 'Add a payment method to receive the first-plan gift, or start the one membership. Final price appears before any charge and follows the account version (rial or dollars).')}</SpecCallout><div className="mo-spec__stack" style={{ marginBlockStart: '1rem' }}><p>✓ {tx(locale, 'یک برنامه ترکیبی در هر دوره', 'One combined plan per period')}</p><p>✓ {tx(locale, 'کارت تا چرخه دوم شارژ نمی‌شود', 'The card is not charged until cycle 2')}</p><p>✓ {tx(locale, 'لغو از تنظیمات عضویت', 'Cancel from membership settings')}</p></div><div className="mo-spec__actions"><SpecButton><CreditCard />{tx(locale, 'افزودن روش پرداخت', 'Add payment method')}</SpecButton><SpecButton kind="secondary">{tx(locale, 'شروع عضویت', 'Start membership')}</SpecButton></div></>
  if (state === 'gift-available') return <><SpecCallout icon={<Gift />} title={tx(locale, 'هدیه برای رزرو در دسترس است', 'Gift is available to reserve')} tone="energy">{tx(locale, 'پیش از ساخت برنامه باید روش پرداخت ثبت شود. رزرو هنوز درخواست ساخت برنامه را مصرف یا سرویس تولید را فراخوانی نمی‌کند و کارت تا چرخه دوم شارژ نمی‌شود.', 'A payment method must be on file before generation. Reserving does not consume the plan request or call the generation provider, and the card is not charged until cycle 2.')}</SpecCallout><div className="mo-spec__actions"><SpecButton>{tx(locale, 'رزرو هدیه برنامه اول', 'Reserve first-plan gift')}</SpecButton><SpecButton kind="secondary">{tx(locale, 'بعداً تصمیم می‌گیرم', 'Decide later')}</SpecButton></div></>
  ...
  if (state === 'region') return <><SpecCallout icon={<Globe2 />} title={tx(locale, 'نسخه روی حساب ثبت شده', 'Version is stored on the account')} tone="brand">{tx(locale, 'IP فعلی یا VPN این فیلد را به‌روز نمی‌کند. ساخت برنامه در هر دو نسخه یکسان است.', 'Current IP or a VPN does not update this field. Generation works the same in both versions.')}</SpecCallout><div className="mo-spec__actions"><SpecButton>{tx(locale, 'تأیید و ساخت برنامه', 'Confirm and create plan')}</SpecButton><SpecButton kind="secondary">{tx(locale, 'مدیریت حساب', 'Manage account')}</SpecButton></div></>
...
export const RegionUnavailable: Story = { name: 'Sticky Iranian account', parameters: momentumEvidence(['LIFE-06'], '/[locale]/onboarding/review', 'in-page'), render: renderLifecycle('region') }
