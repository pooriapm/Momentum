...
function PlanScreen({ locale, view }: { locale: SpecLocale; view: PlanView }) {
  ...
  return (
    <ProductFrame active="plan" locale={locale} title={tx(locale, 'برنامه', 'Plan')}>
      <SpecHeader aside={<SpecBadge tone="brand">{tx(locale, 'دوره ۱ · روز ۱۲', 'Period 1 · Day 12')}</SpecBadge>} body={tx(locale, 'برنامه ۳۰روزه از زمان آماده‌شدن آغاز شده و تا پایان دوره قابل مشاهده است.', 'Your 30-day plan began when it became ready and remains visible through the end of the period.')} eyebrow={tx(locale, 'قدرت و تعادل', 'Strength & balance')} title={tx(locale, 'برنامه من', 'My plan')} />
...
function NutritionView({ locale }: { locale: SpecLocale }) {
  return <div className="mo-spec__stack"><SpecCard tone="energy"><SpecBadge tone="energy">{tx(locale, 'برنامه کامل ۳۰روزه', 'Complete 30-day plan')}</SpecBadge><h2>{tx(locale, 'الگوی ماهانه تغذیه', 'Monthly nutrition pattern')}</h2><p>{tx(locale, 'این دوره از لحظه آماده‌شدن، ۳۰ روز کامل را پوشش می‌دهد. هر وعده چند گزینه هم‌ارزش دارد تا برنامه بدون بازتولید قابل اجرا بماند.', 'This period covers 30 complete days from the moment the plan is ready. Every meal includes equivalent options so the plan remains practical without regeneration.')}</p><SpecList rows={[{ icon: <Check />, tone: 'success', label: tx(locale, 'هفته ۱ · آشنایی با مقدارها', 'Week 1 · Learn portions'), detail: tx(locale, 'گزینه اصلی + ۲ جایگزین برای هر وعده', 'Primary option + 2 alternatives per meal') }, { icon: <Check />, tone: 'success', label: tx(locale, 'هفته ۲ و ۳ · تنوع کنترل‌شده', 'Weeks 2–3 · Guided variety'), detail: tx(locale, 'چرخش پروتئین و سبزیجات با همان هدف روزانه', 'Protein and produce rotation with the same daily target') }, { icon: <Check />, tone: 'success', label: tx(locale, 'هفته ۴ و روزهای باقی‌مانده', 'Week 4 and remaining days'), detail: tx(locale, 'گزینه‌های آشنا تا پایان دقیق دوره', 'Familiar options through the exact period end') }]} /></SpecCard><SpecList rows={[
...
function GroceryView({ locale }: { locale: SpecLocale }) {
  return <div className="mo-spec__grid"><SpecCard className="is-wide"><SpecCallout icon={<ShoppingBasket />} title={tx(locale, 'فهرست برنامه ماهانه', 'Monthly plan list')} tone="energy">{tx(locale, 'مقدارها برای یک نفر و برنامه کامل ۳۰روزه محاسبه شده‌اند. تیک‌ها در حالت آفلاین روی دستگاه ذخیره و پس از اتصال بدون ایجاد مورد تکراری همگام می‌شوند.', 'Quantities cover one person for the complete 30-day plan. Offline checkmarks are stored on this device and sync without duplicates after reconnection.')}</SpecCallout><SpecList rows={[
    ...
    { icon: <Circle />, label: tx(locale, 'برنج قهوه‌ای', 'Brown rice'), detail: tx(locale, '۳٫۲ کیلوگرم', '3.2 kg') },
