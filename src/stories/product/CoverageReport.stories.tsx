...
function CoverageReport({ locale }: { locale: 'fa' | 'en' }) {
  ...
  return (
    <ProductFrame locale={locale} nav={false} title={tx(locale, 'تحویل طراحی', 'Design handoff')}>
      <SpecHeader aside={<SpecBadge tone="energy">{tx(locale, '۱۳۷ شناسه', '137 IDs')}</SpecBadge>} body={tx(locale, '۱۳۲ state قبلی حفظ شده‌اند و پنج state تصمیم D14 اکنون در قرارداد Storybook ثبت شده‌اند. ممیزی کامل بصری D14 هنوز باز است.', 'The prior 132 states remain, and five D14 states are now registered in Storybook. Full D14 visual audit remains open.')} eyebrow={tx(locale, 'گزارش پوشش Storybook', 'Storybook coverage report')} title={tx(locale, 'پوشش طراحی محصول', 'Product-design coverage')} />
