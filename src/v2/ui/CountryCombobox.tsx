...
export function CountryCombobox({ error, label, locale, onChange, suggested, value }: CountryComboboxProps) {
  const id = useId()
  ...
  const results = useMemo(() => {
    ...
    return sortedCountryCodes(locale)
      .filter((code) => !query || code.toLowerCase().includes(query) || countryName(code, locale).toLocaleLowerCase(fa ? 'fa' : 'en').includes(query))
      .slice(0, 14)
  }, [fa, locale, search])
  ...
  return (
    <div className={`orbit-field country-combobox ${error ? 'orbit-field--error' : ''}`} ref={rootRef}>
      ...
      {open ? (
        <div className="country-combobox__menu" id={`${id}-listbox`} role="listbox">
          {results.length ? results.map((code) => (
            <button aria-selected={code === value} key={code} onClick={() => choose(code)} role="option" type="button">
