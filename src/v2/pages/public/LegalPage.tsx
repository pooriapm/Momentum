...
export function LegalPage({ locale, kind }: { locale: AppLocale; kind: 'privacy' | 'terms' }) {
  ...
  const sections = privacy
    ? [
        [fa ? 'چه داده‌ای نگهداری می‌شود؟' : 'What data is stored?', fa ? 'حساب، پروفایل و رضایت‌ها، هدف و ترجیحات، برنامه‌ها و ثبت‌های روزانه، پیام‌های مربی و در صورت انتخاب شما گزارش خصوصی ترکیب بدنی.' : 'Account, profile and consent records, goals and preferences, plans and daily logs, coach messages, and—only if you choose—private body-composition reports.'],
        [fa ? 'چرا پردازش می‌شود؟' : 'Why is it processed?', fa ? 'برای احراز هویت، ساخت و نمایش برنامه، پیگیری، ایمنی، کنترل سهمیه و پاسخ مربی. Momentum داده سلامت را برای تبلیغات هدفمند نمی‌فروشد یا استفاده نمی‌کند.' : 'To authenticate you, create and display plans, track progress, enforce safety and quotas, and answer coaching messages. Momentum does not sell or use health data for targeted advertising.'],
        [fa ? 'پردازش AI' : 'AI processing', fa ? 'در بازارهای مجاز، فقط زمینه لازم از سمت سرور به سرویس‌دهنده AI ارسال می‌شود؛ ایمیل و نام از پرامپت برنامه حذف می‌شوند. سیاست نگهداری سرویس‌دهنده می‌تواند شامل پایش سوءاستفاده تا ۳۰ روز باشد مگر قرارداد دیگری فعال شده باشد.' : 'In supported markets, only necessary context is sent server-side to the AI provider; email and name are excluded from plan prompts. Provider retention may include abuse monitoring for up to 30 days unless a different approved control is enabled.'],
    ...
    : [
        ...
        [fa ? 'شرایط استفاده' : 'Eligibility', fa ? 'استفاده از برنامه‌ریزی خودکار فقط برای افراد ۱۸ سال یا بیشتر، پس از غربالگری ایمنی و در کشور مجاز سرویس‌دهنده امکان‌پذیر است. دورزدن محدودیت جغرافیایی مجاز نیست.' : 'Automated planning is limited to people aged 18 or older who pass safety screening and are in a provider-supported country. Geographic circumvention is not permitted.'],
