...
describe('Me membership and export derivation', () => {
  it('maps one-SKU membership states from the plan', () => {
    ...
    expect(deriveMembershipStatus({ ...demoPlan, progress: { ...demoPlan.progress, entitlementStatus: 'cancelled' } })).toBe('cancelled')
  ...
  it('keeps a single Momentum membership label with no Core/Pro copy', () => {
    ...
    expect(membershipCopy('cancelled', 'en').label).toBe('Cancelled or expired')
