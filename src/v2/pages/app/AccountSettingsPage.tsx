      ...
      <form className="account-settings-form" onSubmit={submit}>
        ...
        <ContentCard className="account-settings-section">
          ...
          <div className="account-settings-grid">
            <Select label={fa ? 'ظاهر' : 'Appearance'} onChange={(e) => { const theme = e.target.value === 'light' ? 'light' : 'dark'; applyUiTheme(theme); updateUiState({ theme }) }} value={document.documentElement.classList.contains('light') ? 'light' : 'dark'}>
              <option value="dark">{fa ? 'تیره' : 'Dark'}</option>
              <option value="light">{fa ? 'روشن' : 'Light'}</option>
            </Select>
            <Select label={fa ? 'نمایش تاریخ' : 'Date display'} onChange={(e) => updatePrefs({ calendar: e.target.value as MePreferences['calendar'] })} value={prefs.calendar}>
              <option value="jalali">{fa ? 'هجری شمسی' : 'Jalali'}</option>
              <option value="gregorian">{fa ? 'میلادی' : 'Gregorian'}</option>
            </Select>
            <Select label={fa ? 'آغاز هفته' : 'First day of week'} onChange={(e) => updatePrefs({ weekStart: e.target.value as MePreferences['weekStart'] })} value={prefs.weekStart}>
