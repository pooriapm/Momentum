...
نسخه‌ی فعلی آلفا و زیر `1.0.0` است. پرداخت زنده هنوز فعال نشده است؛ کشور دسترسی به محصول، هدیه یا generation را مسدود نمی‌کند.
...
- تولید یک برنامهٔ ترکیبی ماهانهٔ کامل با ۳۰ روز مستقل فقط از سمت سرور؛ کلید provider هرگز وارد مرورگر نمی‌شود و محتوای کوتاه‌تر برای پرکردن ماه تکرار نمی‌شود
...
نسخه‌های consent، کلید safety، سقف‌ها، فهرست originهای دقیق و master/feature switchها
را داشته باشد. مقدار نمونه مجوز عرضه نیست.
...
Momentum یک محصول دو‌زبانه و بدون دیوار جغرافیایی است:

- IP فقط در اولین بازدید زبان و مسیر پرداخت را پیشنهاد می‌کند؛ کاربر می‌تواند زبان را هر زمان تغییر دهد؛
- کشور ایران به درگاه ایرانی و قیمت تومان/ریال هدایت می‌شود؛ سایر کشورها به درگاه بین‌المللی و دلار؛
- کشور هیچ‌گاه دسترسی به ثبت‌نام، هدیه، import، alpha، beta یا generation را نمی‌بندد؛
- خرابی provider زنده به خروجی قطعی fallback می‌کند و برنامه معتبر قبلی همیشه حفظ می‌شود؛
- فعال‌سازی هر درگاه همچنان به credential، تأیید webhook، refund و settlement عملیاتی نیاز دارد.
