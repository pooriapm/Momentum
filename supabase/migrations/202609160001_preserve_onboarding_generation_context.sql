-- Preserve structured onboarding answers used by both plan-generation paths.
-- This forward migration replaces the canonical RPC without modifying applied migrations.
alter table public.dietary_preferences
  add column if not exists training_location text
    check (training_location is null or training_location in ('home', 'gym', 'outdoor')),
...
begin
  ...
  if char_length(trim(coalesce(v_payload ->> 'firstName', ''))) not between 1 and 120
    ...
    or coalesce(v_payload ->> 'urgentSymptoms', '') not in ('', 'yes', 'no')
    or coalesce(v_payload ->> 'injuryLimitation', '') not in ('', 'yes', 'no')
    or coalesce(v_payload ->> 'trainingLocation', '') not in ('', 'home', 'gym', 'outdoor')
    or coalesce(v_payload ->> 'trainingExperience', '') not in ('', 'beginner', 'intermediate', 'advanced')
  ...
  v_block_reason := case
    ...
    when v_payload ->> 'urgentSymptoms' = 'yes' then 'urgent_symptoms'
    else null
  end;
  ...
  insert into public.dietary_preferences(
    ...
    nullif(v_payload ->> 'trainingLocation', ''),
    nullif(v_payload ->> 'trainingExperience', '')
  )
  ...
  values (
    ...
    array_remove(array[
      ...
      case when v_payload ->> 'injuryLimitation' = 'yes' then 'injury_or_movement_limitation' end
    ], null),
    array(
      select left(trim(item), 160)
