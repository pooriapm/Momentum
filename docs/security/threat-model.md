...
body-composition reports, plans and logs, monthly plan snapshots, OpenAI/service
...
| Prompt injection from profile/cycle note | Context serialized as data, bounded note, explicit untrusted-data rule, no model-controlled DB tools, strict output schema | Model can still generate unsafe advice; safety evals and monitoring required |
...
| Geo/jurisdiction manipulation | Signup IP writes sticky `product_region` once; later IP cannot switch version or currency. Raw IP is not stored | Admin correction of `product_region` is audited |
...
context, prompts, model output, cycle-note content, service keys and provider error
