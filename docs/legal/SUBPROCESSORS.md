...
| OpenAI API | AI plan generation, coach replies and optional report extraction | minimized structured wellness context, user message, report content when requested, generated output, pseudonymous safety identifier and usage metadata | Server integration is feature-switched off by default | Supported market, written contractual/legal approval, DPA/data controls, retention mode, safety evals, cost limit, current subprocessors; Iran remains blocked |
...
| Product analytics — `[NOT_SELECTED]` | aggregate product measurement | pseudonymous event name and coarse product state; never raw health values, reports, or coach text | Not approved | Consent/lawful-basis decision, event allowlist, DPA, retention, opt-out and deletion behavior |
